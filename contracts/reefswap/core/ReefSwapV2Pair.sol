...
contract ReefSwapV2Pair is IReefSwapV2Pair, ReefSwapV2ERC20 {
    ...
    function initialize(address _token0, address _token1) external override {
        require(msg.sender == factory, 'ReefSwapV2: FORBIDDEN');
        token0 = _token0;
        token1 = _token1;
    }

    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= type(uint112).max && balance1 <= type(uint112).max, 'ReefSwapV2: OVERFLOW');
        ...
        if (timeElapsed > 0 && _reserve0 != 0 && _reserve1 != 0) {
            // price accumulators intentionally overflow
            unchecked {
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external override lock {
        ...
        uint balance1;
        {
            ...
            if (data.length > 0)
                IReefSwapV2Callee(to).reefswapV2Call(msg.sender, amount0Out, amount1Out, data);
            balance0 = IERC20Minimal(_token0).balanceOf(address(this));
            balance1 = IERC20Minimal(_token1).balanceOf(address(this));
        }
